...
@test "bootstrap script exists and twf help works in clean container" {
...
@test "twf add works in clean container workspace" {
  ...
  run_twf add
  [ "$status" -eq 0 ]
  run test -f "$TEAM_ROOT/docker-project/project.yml"
  [ "$status" -eq 0 ]
}
